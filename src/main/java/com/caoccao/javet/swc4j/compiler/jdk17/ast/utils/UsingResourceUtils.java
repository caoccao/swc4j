...
/**
 * Utility class for generating using-declaration resource cleanup bytecode.
 */
public final class UsingResourceUtils {
    private UsingResourceUtils() {
    }

    static void generateCloseWithSuppression(
    ...
    static void generateDeclaratorChain(
            ByteCodeCompiler compiler,
            CodeBuilder code,
        ...
        if (!(name instanceof Swc4jAstBindingIdent bindingIdent)) {
            throw new Swc4jByteCodeCompilerException(sourceCode, declarator,
        ...
        validateAutoCloseableType(compiler, varType, declarator);
        ...
        if (declarator.getInit().isPresent()) {
            ...
            ReturnTypeInfo varTypeInfo = ReturnTypeInfo.of(sourceCode, declarator, localVar.type(), null);
        ...
        if (idx + 1 < decls.size()) {
            ...
            generateDeclaratorChain(compiler, code, classWriter, decls, idx + 1, remainingStmts, returnTypeInfo);
        ...
        if (tryStartPc < tryEndPc) {
            ...
            if (!tryEndsWithTerminal) {
                generateInlineClose(code, classWriter, localVar.index());
            ...
            generateCloseWithSuppression(compiler, code, classWriter, localVar.index(), tempException.index());
     ...
     * Generates null-safe AutoCloseable.close() call.
     *
     * @param code         the code builder
     * @param classWriter  the class writer
     * @param resourceSlot the local variable slot of the resource
     */
    public static void generateInlineClose(CodeBuilder code, ClassWriter classWriter, int resourceSlot) {
        var cp = classWriter.getConstantPool();
        // aload <resource>
        code.aload(resourceSlot);
     ...
     */
    public static void generateWithRemainingStatements(
            ByteCodeCompiler compiler,
            CodeBuilder code,
            ClassWriter classWriter,
        ...
        if (usingDecl.isAwait()) {
            throw new Swc4jByteCodeCompilerException(sourceCode, usingDecl,
        ...
        generateDeclaratorChain(compiler, code, classWriter, decls, 0, remainingStmts, returnTypeInfo);
    }

    static void validateAutoCloseableType(
            ByteCodeCompiler compiler, String varType, Swc4jAstVarDeclarator declarator)
