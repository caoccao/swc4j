...
public final class ClassProcessor extends BaseAstProcessor<Swc4jAstClass> {
    ...
    @Override
    public void generate(CodeBuilder code, ClassWriter classWriter, Swc4jAstClass clazz, ReturnTypeInfo returnTypeInfo) throws Swc4jByteCodeCompilerException {
        String currentPackage = compiler.getMemory().getScopedPackage().getCurrentPackage();
        String className = clazz.getParent(Swc4jAstClassDecl.class).orElseThrow(() ->
                new Swc4jByteCodeCompilerException(getSourceCode(), clazz, "Failed to find parent class declaration")).getIdent().getSym();
        ...
        classWriter = new ClassWriter(internalClassName, superClassInternalName);
        ...
        try {
            ...
            for (ISwc4jAstClassMember member : clazz.getBody()) {
                if (member instanceof Swc4jAstClassMethod classMethod) {
                    compiler.getClassMethodProcessor().generate(null, classWriter, classMethod, null);
                } else if (member instanceof Swc4jAstPrivateMethod privateMethod) {
                    // ES2022 private methods (#method)
                    compiler.getPrivateMethodProcessor().generate(null, classWriter, privateMethod, null);
                }
            }
            ...
            throw new Swc4jByteCodeCompilerException(getSourceCode(), clazz, "Failed to generate bytecode for class: " + fullClassName, e);
        } finally {
    ...
    private void generateConstructorWithFieldInit(
            ClassWriter classWriter,
            String internalClassName,
     ...
     */
    private void generateExplicitConstructor(
            ClassWriter classWriter,
            String internalClassName,
            String superClassInternalName,
