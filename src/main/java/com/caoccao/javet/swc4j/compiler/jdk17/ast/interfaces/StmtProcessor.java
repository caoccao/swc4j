...
public final class StmtProcessor extends BaseAstProcessor<ISwc4jAstStmt> {
    ...
    public void generate(CodeBuilder code, ClassWriter classWriter, ISwc4jAstStmt stmt, ReturnTypeInfo returnTypeInfo) throws Swc4jByteCodeCompilerException {
        if (stmt instanceof Swc4jAstTsModuleDecl tsModuleDecl) {
            compiler.getTsModuleDeclProcessor().generate(code, classWriter, tsModuleDecl, returnTypeInfo);
        } else if (stmt instanceof Swc4jAstClassDecl classDecl) {
            compiler.getClassDeclProcessor().generate(code, classWriter, classDecl, returnTypeInfo);
        } else if (stmt instanceof Swc4jAstTsEnumDecl tsEnumDecl) {
            compiler.getTsEnumDeclProcessor().generate(code, classWriter, tsEnumDecl, returnTypeInfo);
        } else if (stmt instanceof Swc4jAstTsInterfaceDecl tsInterfaceDecl) {
            compiler.getTsInterfaceDeclProcessor().generate(code, classWriter, tsInterfaceDecl, returnTypeInfo);
        }
    }
}
