                ...
                case "substr" -> {
                    ...
                    if (callExpr.getArgs().isEmpty()) {
                        ...
                        if (callExpr.getArgs().size() > 1) {
                            ...
                            // In JavaScript, substr(start) extracts from start to end
                            // We need to calculate length = str.length() - start
                            // But we need the string reference for length calculation
                            // Easiest approach: call substr with a very large length
