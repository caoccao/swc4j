...
public class Swc4jAstTsQualifiedName
    ...
    public boolean replaceNode(ISwc4jAst oldNode, ISwc4jAst newNode) {
        if (left == oldNode && newNode instanceof ISwc4jAstTsEntityName newLeft) {
            setLeft(newLeft);
            return true;
        }
        if (right == oldNode && newNode instanceof Swc4jAstIdentName newRight) {
            setRight(newRight);
            return true;
        }
        return false;
    ...
    public Swc4jAstVisitorResponse visit(ISwc4jAstVisitor visitor) {
        return switch (visitor.visitTsQualifiedName(this)) {
