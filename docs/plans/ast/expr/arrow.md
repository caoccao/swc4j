...
Each arrow expression compiles to an anonymous inner class that:
1. Implements a functional interface (SAM - Single Abstract Method)
2. Stores captured variables as final fields
3. Has a constructor to initialize captured values
4. Implements the abstract method with the arrow body
...
class Counter {
  ...
  Supplier<Integer> createIncrementer() {
    return new Counter$Lambda$1(this);
  }
}

// Anonymous inner class
class Counter$Lambda$1 implements Supplier<Integer> {
...
- `OuterClass$Lambda$N` where N is a sequential counter
